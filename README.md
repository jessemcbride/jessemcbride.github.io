...
The account is **Nonduality**, formerly **I am a bot**. The homepage links to **bank.html**, a separate pseudo bank filled with collection-log item icons and recorded quantities. Search by name, switch between all finds, keepsakes and recent notable finds, sort the collection, or select an item for a closer look. The bank is a collection-log display, not a claim about current inventory. Skills and bosses stay in collapsed notes on the bank page. Skills and boss counts come from [Wise Old Man](https://docs.wiseoldman.net/api/players/player-endpoints). Each scheduled refresh updates the current username before reading its snapshot, avoiding an old record belonging to a previous name holder. The source timestamp is shown on the site.
