...
export function osrs(feed = {}, log = {}, name = 'Nonduality', note = '', now = Date.now()) {
  ...
  // A slow procession of verified finds, never inferred from kill counts.
  const featured = [...new Set([23760, ...(log.notable || []).map(i => i.id), 11920, 6737, 23956, 25582, 22994, ...collection.map(i => i.id)])].map(id => byId.get(id)).filter(Boolean).slice(0, 16);
